...
// Adapter for UUID format...
const adaptUuid = uuid => {
  const p = (a, b, dash = true) => uuid.substring(a, b) + (dash ? '-' : '')
  return p(0, 8) + p(8, 12) + p(12, 16) + p(16, 20) + uuid.substring(20, uuid.length, true)
}

async function open(deviceOrId: Device, needsReconnect: boolean) {
  // Handel Device Id
  const device = deviceOrId
  // console.log(getBluetoothServiceUuids())
  const promise = new Promise((resolve, reject) => {
    console.log('[node-ble] >> Trying to connect')
    device.connect(err => {
      if (err) {
        device.disconnect()
        return reject(err)
      }
      console.log('[node-ble] >> Device connected')
      console.log('[node-ble] >> Trying to discover services')
      device.discoverServices([], (err, services) => {
        console.log('[node-ble] >> Discovered services')
        if (err) {
          device.disconnect()
          return reject(err)
        }
        const service = services[0]
        const uuid = adaptUuid(service.uuid)
        const infos = getInfosForServiceUuid(uuid)
        const { deviceModel, writeUuid, notifyUuid } = infos
        console.log('[node-ble] >> Trying to discover characteristics')
        service.discoverCharacteristics([], (err, characteristics) => {
          console.log('[node-ble] >> Discovered characteristics')
          if (err) {
            device.disconnect()
            return reject(err)
          }
          const writeC = characteristics.find(cha => adaptUuid(cha.uuid) === writeUuid)
          const notifyC = characteristics.find(cha => adaptUuid(cha.uuid) === notifyUuid)

          const subscribe = () => {
            let timeout = setTimeout(() => notifyC.subscribe(), 5000)
            notifyC.subscribe(async err => {
              clearTimeout(timeout)
              if (err) {
                device.disconnect()
                return reject(err)
              }
              const notifyObservable = monitorCharacteristic(notifyC).pipe(tap(value => { logSubject.next({ type: 'ble-frame-read', message: value.toString('hex') }) }), share())
              ...
              try {
                ...
                if (afterMTUTime - beforeMTUTime < 500) needsReconnect = false; // (optim) there is likely no new pairing done because mtu answer was fast.
                if (needsReconnect) {
                  console.log('[node-ble] >> Nano needs to reconnect (infetMTU)')
...
export default class BluetoothTransport extends Transport<Device | string> {
  ...
  write = async (buffer: Buffer) => {
    ...
    this.writeCharacteristic.write(buffer, false, (err, data) => { if (err) throw err })
  };
